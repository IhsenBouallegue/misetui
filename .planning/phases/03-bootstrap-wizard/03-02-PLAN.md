...
must_haves:
  truths:
    ...
    - "Wizard calls async detect_project_tools() (which uses mise ls -J for installed status)"
    ...
    - "Pressing Enter in Preview step triggers file write + mise install via install_project_tools()"
  ...
  artifacts:
    ...
    - path: "src/main.rs"
      ...
      contains: "OpenWizard"
    - path: "src/mise.rs"
      provides: "write_mise_toml(), write_agent_files_for() — install uses existing install_project_tools()"
...
**src/mise.rs** — Append **two** new async functions at the end of mise.rs.

Note: `run_mise_install_in` is NOT needed — the existing `install_project_tools(path)` function already does this (runs `mise install` in a given directory). The wizard's write step calls `install_project_tools` directly.
...
<verification>
Run `cargo check` — zero errors. Grep for `OpenWizard` in src/action.rs, src/app.rs, src/main.rs — must appear in all three. Grep for `write_mise_toml` in src/mise.rs — must be present. Grep for `detect_project_tools` in src/app.rs — spawn must use `.await`.
</verification>
...
- write_mise_toml and write_agent_files_for added to mise.rs (install reuses existing install_project_tools)
- detect_project_tools spawned as async (not blocking)
</success_criteria>
